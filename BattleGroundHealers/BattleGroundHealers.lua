...
local DefaultSettings = {
    ...
    GBFStracking = 1,           -- Detect healers via BG Scoreboard (1 = enabled, 0 = disabled)
    h2dRatio = 2.5,             -- BG Scoreboard healing-to-damage ratio threshold (1 to 5)
    healingThreshold = 30000,   -- BG Scoreboard healing detection threshold (10k to 100k)
...
local LHCN = {}
for _, classType in ipairs(HealerClasses) do
    LHCN[LOCALIZED_CLASS_NAMES_MALE[classType]] = true
    LHCN[LOCALIZED_CLASS_NAMES_FEMALE[classType]] = true
end
...
local anchorMapping = {
    ["left"] = {
        anchorPoint = "RIGHT", relativePoint = "CENTER",
        xOffset = -70.5, yOffset = -9
    },
    ["top"] = {
        anchorPoint = "BOTTOM", relativePoint = "CENTER",
        xOffset = 0, yOffset = 11
    },
    ["right"] = {
        anchorPoint = "LEFT", relativePoint = "CENTER",
        xOffset = 51, yOffset = -9
    },
}
...
local function UpdateTextures(self)
    local texture = MarkedNames[self.activeName]
    local levelRegion = self.levelRegion
    if texture then
        if texture ~= self.prevTexture then
            self.mark:SetTexture(texture)
            self.mark:Show()
            self.prevTexture = texture      
        end
        if BGHsettings.iconAnchor == "right" then
            -- Hide level text if it overlaps with the icon
            ...
            if BGHsettings.iconStyle == "Blizzlike" then
                ...
                if d < r or R < 1 then
                    self.levelRegion:Hide()
                else
                    self.levelRegion:Show()
                end
        ...
        self.mark:Hide()
        self.prevTexture = nil
        self.levelRegion:Show()
    end
end

------------ Update all mark frames when the configuration changes  ------------
local function UpdateGlobalMarks()
    for _, frame in pairs(GlobalNamePlates) do
        if frame.mark then
            frame.mark:ClearAllPoints()
            local anchorData = anchorMapping[BGHsettings.iconAnchor or "left"]
            frame.mark:SetPoint(
            ...
            frame.mark:SetSize(BGHsettings.iconSize, BGHsettings.iconSize)
...
local function NamePlate_OnShow(self)
    local name = self.nameRegion:GetText()
    self.activeName = name
    ActiveNamePlates[name] = self
...
local function NamePlate_OnHide(self)
    self.mark:Hide()
    ActiveNamePlates[self.activeName], self.activeName, self.prevTexture = nil
end

local function NamePlate_OnUpdate(self)
    if self.nameRegion:GetText() ~= self.activeName then
        NamePlate_OnHide(self)
        NamePlate_OnShow(self)
        self.activeName = self.nameRegion:GetText()
    end
end

-------- Setup a frame that manages the mark texture parameters  --------
local function SetupNamePlate(self)
    local frame = CreateFrame("Frame", nil, self)
    GlobalNamePlates[self] = frame
    frame:SetFrameStrata("TOOLTIP")
    frame.mark = self:CreateTexture(nil, "OVERLAY")
    frame.mark:ClearAllPoints()
    local anchorData = anchorMapping[BGHsettings.iconAnchor or "left"]
    frame.mark:SetPoint(
        anchorData.anchorPoint,
    ...
    frame.mark:SetSize(BGHsettings.iconSize, BGHsettings.iconSize)
    local _, _, _, _, _, _, nameRegion, levelRegion = self:GetRegions()
    frame.nameRegion = nameRegion
    frame.activeName = nameRegion:GetText()
    ActiveNamePlates[frame.activeName] = self
    frame.levelRegion = levelRegion
    frame.levelRegion:SetDrawLayer("ARTWORK")
...
local function IsNamePlate(self)
    ...
    if self.GetRegions then
        ...
        if region and region.GetTexture then
            return region:GetTexture() == [[Interface\TargetingFrame\UI-TargetingFrame-Flash]]
        end
    end
end

------ Script to detect when the number of nameplates in the WorldFrame increases  ------
local prevChildCount = 1
CreateFrame("Frame"):SetScript("OnUpdate", function(self, elapsed)
    if prevChildCount ~= WorldFrame:GetNumChildren() then
        for _, child in next, { WorldFrame:GetChildren() }, prevChildCount do
...
---------- Updates the list of healers based on the BG Scoreboard, prioritizing the Combat Log healers list if active ----------
local function UpdateGBFShealers()
    if BGHsettings.GBFStracking == 1 then
        ...
        for i = 1, GetNumBattlefieldScores() do
            ...
            if name then
                ...
                if name == playerName and not playerFaction then
                    playerFaction = faction
                end
                if CLEUhealers[name] then
                    if GBFShealers[name] then
                        GBFShealers[name] = nil
                        if debugMode then
                ...
                elseif healingDone > BGHsettings.h2dRatio * damageDone and healingDone > BGHsettings.healingThreshold and LHCN[class] then
                    if not GBFShealers[name] then
                        GBFShealers[name] = faction
                        if debugMode then
                            print(string.format("|cff00FF98[BGH] Debug:|r %s (%s) added to BG Scoreboard healers list.", name, faction == 1 and "Alliance" or "Horde"))
                        end 
                    end
                elseif GBFShealers[name] then
                    GBFShealers[name] = nil
                    SetBGHmark(name, nil)
                    if debugMode then
        ...
        for name in pairs(GBFShealers) do
            if not currentBGplayers[name] then
                print(string.format("|cff00FF98[BGH]|r %s (%s) has left the BG, removed from healers list.", name, GBFShealers[name] == 1 and "Alliance" or "Horde"))
                GBFShealers[name] = nil
                SetBGHmark(name, nil)
            end
        end
        if playerFaction then
            for name, faction in pairs(GBFShealers) do
    ...
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        if subEvent == "SPELL_CAST_SUCCESS" or subEvent == "SPELL_AURA_APPLIED" then
            if healerSpellHash[spellID] then
                local cleanName = sourceName:match("([^%-]+).*")
                if not CLEUhealers[cleanName] then
                    ...
                    for i = 1, GetNumBattlefieldScores() do
                        ...
                        if name then
                            ...
                            if name == cleanName and LHCN[class] and not sourceFaction then
                    ...
                    if sourceFaction and playerFaction then
                        CLEUhealers[cleanName] = sourceFaction
                        SetBGHmark(cleanName, ((BGHsettings.iconInvertColor == 1) == (sourceFaction == playerFaction)) and IconTextures[BGHsettings.iconStyle].Red or IconTextures[BGHsettings.iconStyle].Blue)
                        if debugMode then
                            print(string.format("|cff00FF98[BGH] Debug:|r %s (%s) added to Combat Log healers list (spellID: %s)", cleanName, sourceFaction == 1 and "Alliance" or "Horde", spellID))
                        end 
                    end
                end
            end
...
local function ConfigUI()
    if not BGHConfigUIglobalFrame then
        local ConfigUIFrame = CreateFrame("Frame", "BGHConfigUIglobalFrame", UIParent, "UIPanelDialogTemplate")
        ConfigUIFrame:SetScale(0.9/UIParent:GetScale())
        ConfigUIFrame:SetSize(280, 545)
        ConfigUIFrame:SetPoint("CENTER")
        ConfigUIFrame:Show()
        ConfigUIFrame.title:SetText("BattleGroundHealers")
        ConfigUIFrame.title:ClearAllPoints()
        ConfigUIFrame.title:SetPoint("TOP", ConfigUIFrame, "TOP", 0, -9)
        _G[ConfigUIFrame:GetName() .. "DialogBG"]:SetAlpha(0.85)
        _G[ConfigUIFrame:GetName() .. "Close"]:SetScript("OnClick", function()
            ConfigUIFrame:Hide()
            ConfigUIFrame:ClearAllPoints()
            ConfigUIFrame:SetScale(0.9/UIParent:GetScale())
        end)

        local function CreateSeparatorLine(subtitle)
            local lineWidth = (ConfigUIFrame:GetWidth() - subtitle:GetStringWidth() - 49)/2
        ...
        ConfigUIFrame.subtitle1:SetPoint("TOP",0,-44)
        ...
        CLEUfixCheckbox:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 55, -93)
        ...
        CLEUtrackingCheckbox:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 30, -68)
        ...
        local GBFStrackingCheckbox = CreateFrame("CheckButton", "BGHConfigUIGBFStrackingCheckbox", ConfigUIFrame, "UICheckButtonTemplate")
        GBFStrackingCheckbox:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 30, -118)
        GBFStrackingCheckbox:SetSize(24, 24)
        GBFStrackingCheckbox.Text = _G[GBFStrackingCheckbox:GetName() .. "Text"]
        GBFStrackingCheckbox.Text:SetText("Track Healers via BG Scoreboard")
        GBFStrackingCheckbox.Text:SetPoint("LEFT", GBFStrackingCheckbox, "RIGHT", 1, 1) 
        GBFStrackingCheckbox.Text:SetTextColor(1, 1, 1, 1)
        GBFStrackingCheckbox:SetChecked(BGHsettings.GBFStracking == 1)
        GBFStrackingCheckbox:SetScript("OnClick", function(self)
            if self:GetChecked() then
                BGHsettings.GBFStracking = 1
                if inBG then
                ...
                BGHsettings.GBFStracking = 0
                ClearHealers(GBFShealers)
        ...
        printChannelDropdown:SetPoint("TOPRIGHT", ConfigUIFrame, "TOPRIGHT", -14, -152)
        ...
        printHealersButton:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 30, -152)
        ...
        ConfigUIFrame.subtitle2:SetPoint("TOP",0,-195)
        ...
        iconSizeSlider:SetPoint("TOP", ConfigUIFrame, "TOP", 0, -233)
        ...
        iconStyleDropdown:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 14, -283)
        ...
        iconXoffsetSlider:SetPoint("TOP", ConfigUIFrame, "TOP", 0, -334)
        ...
        iconYoffsetSlider:SetPoint("TOP", ConfigUIFrame, "TOP", 0, -383)
        ...
        iconAnchorDropdown:SetPoint("TOPRIGHT", ConfigUIFrame, "TOPRIGHT", -14, -283)
        ...
        UIDropDownMenu_SetText(iconAnchorDropdown, BGHsettings.iconAnchor or "left")
        ...
        iconInvertColorCheckbox:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 30, -423)
        ...
        testMarkButton:SetPoint("TOPRIGHT", ConfigUIFrame, "TOPRIGHT", -30, -451)
        ...
        testModeCheckbox:SetPoint("TOPLEFT", ConfigUIFrame, "TOPLEFT", 30, -453)
        ...
        resetButton:SetScript("OnClick", function()
            StaticPopupDialogs["CONFIRM_RESET_BGH_CONFIG"] = {
                ...
                OnAccept = function()
                    ...
                    if BGHsettings.GBFStracking == 1 then
                        GBFStrackingCheckbox:SetChecked(true)   
                        if inBG then
                            RequestBattlefieldScoreData()
                            UpdateGBFShealers()
                        end
                    else
                        GBFStrackingCheckbox:SetChecked(false)  
                        ClearHealers(GBFShealers)
                    end  
