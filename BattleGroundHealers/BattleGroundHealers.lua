...
local function UpdateTextures(BGHregion)
    ...
    if texture then
        ...
        if BGHsettings.iconAnchor == "right" and not TidyPlatesCheck then
            -- Hide Blizz plate's level text if it overlaps with the icon
...
local function SetupNamePlate(plate)
    ...
    if plate.extended then -- Using TidyPlates custom frames if available
        ...
        if not TidyPlatesCheck then
            print("|cff00FF98[BGH]|r TidyPlates detected, placing icons on custom nameplates")
...
local function IsNamePlate(frame)
    ...
    if frame.GetRegions then
        ...
        if region and region:GetTexture() == "Interface\\Tooltips\\Nameplate-Border" then
            return true
        end
    end
end
    ...
    if prevChildCount ~= WorldFrame:GetNumChildren() then
        for _, ChildFrame in next, { WorldFrame:GetChildren() }, prevChildCount do
            if IsNamePlate(ChildFrame) then
                -- Short delay to ensure TidyPlates' plate.extended is available --
                local delay = 0
                CreateFrame("Frame"):SetScript("OnUpdate", function(self, elapsed)
                    delay = delay + elapsed
                    if delay > 0.1 then
