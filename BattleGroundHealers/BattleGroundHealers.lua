...
local DefaultSettings = {
    ...
    iconAnchor = "left",        -- Icon anchor relative to the nameplate ("left", "top" or "right")
...
local function UpdateGBFShealers()
    if BGHsettings.GBFStracking == 1 then
        ...
        for i = 1, GetNumBattlefieldScores() do
            ...
            if name then
                name = name:match("([^%-]+).*")
                if healingDone > BGHsettings.h2dRatio * damageDone and healingDone > BGHsettings.healingThreshold and LHCN[class] then
                    if not GBFShealers[name] and not CLEUhealers[name] and playerFaction then
    ...
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        if subEvent == "SPELL_CAST_SUCCESS" or subEvent == "SPELL_AURA_APPLIED" then
            if healerSpellHash[spellID] then
                ...
                if not CLEUhealers[sourceName] then
                    for i = 1, GetNumBattlefieldScores() do
                        local name, _, _, _, _, faction, _, _, class = GetBattlefieldScore(i)
                        if name then
                            name = name:match("([^%-]+).*")
                            if name == sourceName and LHCN[class] then
