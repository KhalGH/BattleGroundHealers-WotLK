...
local HEX_COLOR_HORDE = "|cffe50d0d"
        ...
        for name, data in pairs(healersList) do
            if not currentBGplayers[name] then
                if data then
                    ...
                    BGH_print(string.format(L["%s %s has left the BG, removed from healers list."], coloredName, factionText))
                else
                    BGH_print(string.format(L["%s has left the BG, removed from healers list."], name))
                end
...
local function PrintDetectedHealers()
    ...
    local function AppendHealers(healersList)
        for name, data in pairs(healersList) do
            ...
            if data.faction == 1 then
                ...
                table.insert(allianceHealersColored, coloredName)
            else
                table.insert(hordeHealers, name)
                table.insert(hordeHealersColored, coloredName)
            end
        end
    end
    if BGHsettings.WSSFtracking == 1 then
    ...
    if printChannel then
        ...
        SendChatMessage(string.format(" - %d %s: %s", #allianceHealers, L["Alliance"], table.concat(allianceHealers, ", ")), printChannel)
        SendChatMessage(string.format(" - %d %s: %s", #hordeHealers, L["Horde"], table.concat(hordeHealers, ", ")), printChannel)
    else
        print("|cff00FF98================ BattleGroundHealers ================|r")
        print(string.format(" " .. L["Healers detected in %s:"], "|cffffd100" .. BGName .. "|r"))
        print(string.format("%s  - %d %s:|r %s", HEX_COLOR_ALLIANCE, #allianceHealersColored, L["Alliance"], table.concat(allianceHealersColored, ", ")))
        print(string.format("%s  - %d %s:|r %s", HEX_COLOR_HORDE, #hordeHealersColored, L["Horde"], table.concat(hordeHealersColored, ", ")))
        print("|cff00FF98==================================================|r")
    end
    ...
    if not BGHConfigUIglobalFrame then
        ...
        CLEUfixCheckbox:SetScript("OnClick", function(self)
            if self:GetChecked() then
                BGH_print(L["Automatic Combat Log Fix"], HEX_GREEN .. L["Enabled"] .. "|r")
                BGHsettings.CLEUfix = 1
            else
                BGH_print(L["Automatic Combat Log Fix"], HEX_RED .. L["Disabled"] .. "|r")
        ...
        CLEUtrackingCheckbox:SetScript("OnClick", function(self)
            if self:GetChecked() then
                BGH_print(L["Combat Log tracking"], HEX_GREEN .. L["Enabled"] .. "|r")
                ...
                BGH_print(L["Combat Log tracking"], HEX_RED .. L["Disabled"] .. "|r")
        ...
        WSSFtrackingCheckbox:SetScript("OnClick", function(self)
            if self:GetChecked() then
                BGH_print(L["BG Scoreboard tracking"], HEX_GREEN .. L["Enabled"] .. "|r")
                ...
                BGH_print(L["BG Scoreboard tracking"], HEX_RED .. L["Disabled"] .. "|r")
        ...
        testModeCheckbox:SetScript("OnClick", function(self)
            if self:GetChecked() then
                BGH_print(L["Test mode"], HEX_GREEN .. L["Enabled"] .. "|r")
                testMarkButton:Enable()
            else
                BGH_print(L["Test mode"], HEX_RED .. L["Disabled"] .. "|r")
                testMarkButton:Disable()
...
SlashCmdList["BGH"] = function(msg)
    ...
    elseif cmd == "debug" then
        if debugMode then 
            debugMode = false
            BGH_print("Debug mode " .. HEX_RED .. "Disabled" .. "|r")
        else 
            debugMode = true
            BGH_print("Debug mode " .. HEX_GREEN .. "Enabled" .. "|r")
        end
    end
end
